...
| ft_write   | ssize_t ft_write(int fd, const void *buf, size_t count); | wraps `write(2)`, ignores `SIGPIPE`, sets `errno` |
...
### Behavior notes
- System V AMD64 ABI throughout; registers/stack are preserved where required.
- `ft_write` sets `errno` from `__errno_location` and masks `SIGPIPE` so a closed pipe yields `EPIPE` instead of terminating the process.
